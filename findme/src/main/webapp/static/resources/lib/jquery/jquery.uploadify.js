...
(function($) {
	...
	var handlers = {
		...
		onDialogOpen : function() {
			...
			this.queueData.errorMsg       = '部分文件没有被添加到上传队列中:';
		...
		onSelect : function(file) {
			...
			for (var n in this.queueData.files) {
				...
				if (queuedFile.uploaded != true && queuedFile.name == file.name) {
					var replaceQueueItem = confirm('文件 "' + file.name + '" 已在队列中或已上传.\n是否要覆盖队列中的文件?');
				...
				switch(errorCode) {
					case SWFUpload.QUEUE_ERROR.QUEUE_LIMIT_EXCEEDED:
						if (settings.queueSizeLimit > errorMsg) {
							this.queueData.errorMsg += '\n文件上传量已超过最大限量 (' + errorMsg + ').';
						} else {
							this.queueData.errorMsg += '\n文件上传量已超过最大队列数量 (' + settings.queueSizeLimit + ').';
						}
						break;
					...
					case SWFUpload.QUEUE_ERROR.ZERO_BYTE_FILE:
						...
						break;
					case SWFUpload.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT:
						this.queueData.errorMsg += '\n文件 "' + file.name + '" 不是正确的文件格式 (' + settings.fileTypeDesc + ').';
						break;
				}
			}
		...
		onUploadError : function(file, errorCode, errorMsg) {
			...
			switch(errorCode) {
				...
				case SWFUpload.UPLOAD_ERROR.UPLOAD_LIMIT_EXCEEDED:
					alert('已超过最大上传限量 (' + errorMsg + ').');
					errorString = '超过最大上传限量';
					break;
			...
			if (settings.checkExisting) {
				$.ajax({
					...
					success : function(data) {
						if (data == 1) {
							var overwrite = confirm('服务器上已经有一个文件名为 "' + file.name + '" 德文件.\n是否要覆盖原有文件?');
							if (!overwrite) {
								this.cancelUpload(file.id);
								$('#' + file.id).remove();
	...
	$.fn.uploadify = function(method) {
		...
		if (methods[method]) {
			...
			$.error('在$.uploadify中找不到方法 ' + method);
		}
